...
void search(char cawal, char ctujuan, simpul *awal, int beban, char path[], int index, int panjang[])
{
    ...
    if (awal != NULL)
    {
        ...
        while (bantu_jalur != NULL)
        {
            ...
            {
                panjang[index] = beban;
                if (panjang[index + 1] >= beban)
                {

                    printf("%d\n", panjang[index]); // print node selanjutnya sampai tujuan
                }
            }

            search(cawal, ctujuan, bantu_jalur->tujuan, beban, path, index, panjang); // rekursif
